...
a_star :: Grid -> Position -> [Position]
a_star grid end = rec_a_star grid (heuristic grid end) (singleton end) empty (insert end 0 $ infinityMap grid) (insert end 0 $ infinityMap grid)
    where
...
-- Parts
part2 :: (Grid, Position, Position) -> Int
part2 (grid, _, end) = length (a_star grid end) - 1
