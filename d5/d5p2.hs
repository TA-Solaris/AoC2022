...
runCommand (Command amount from to) stacks = replaceInList (snd (splitAt amount (stacks !! from))) from (replaceInList ((fst (splitAt amount (stacks !! from))) ++ (stacks !! to)) to stacks)
